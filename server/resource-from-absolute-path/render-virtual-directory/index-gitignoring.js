...
/* refactor
  ...
  include study.json or lenses.json directly as a .study property on directory objects
...
const renderVirtualDirectory = async ({
  ...
  if (paths.includes('.gitignore')) {
    gitignore = parseGitignore(
      fs.readFileSync(path.join(absolutePath, '.gitignore'), 'utf8'),
    );
  }
