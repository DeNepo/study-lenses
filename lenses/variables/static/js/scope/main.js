...
if ({}.hasOwnProperty.call(params, "code")) {
  if (
    {}.hasOwnProperty.call(params, "type") &&
    (params.type === "Module" || params.type === "Script")
  ) {
...
function mouseOverHandler(e) {
  ...
  while (
    current &&
    !(
      current.classList.contains("code-binding") ||
      current.classList.contains("code-identifier")
    )
  ) {
...
function wrapIdentifiersInSpans(program, identifiers, locations) {
  ...
  for (var i = 0; i < identifiers.length; ++i) {
    ...
    out +=
      '<span class="code-binding" data-identifier="' +
      i +
      '">' +
      escapeHTML(idStr.substring(0, trailingWhitespaceIndex)) +
      "</span>" +
      idStr.substring(trailingWhitespaceIndex);
...
function onChange() {
  ...
  var parseFn = radio.checked
    ? parser.parseScriptWithLocation
    : parser.parseModuleWithLocation;
  try {
    ...
    var wrappedCode = wrapIdentifiersInSpans(code, identifiers, locations);
    var lineNumbered = wrappedCode
      .split("\n")
      .map((line, number) => {
        number = number + 1;
        if (number < 10) {
