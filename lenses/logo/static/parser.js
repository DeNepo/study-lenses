class Parser {
  ...
  nextToken() {
    ...
    while (
      regWhitespace.test(this.text.charAt(this.index)) &&
    ...
    while (
      ((regWhitespace.test(actualChar) && isTokenList) ||
        !regWhitespace.test(actualChar)) &&
      this.remainingTokens()
    ) {
      this.index++;
      ...
      if (cmd) {
        for (let i = 0; i < cmd.argsTemplate.length; i++) {
          ...
          if (arg.validator !== undefined) {
            ...
            } else valid = arg.validator(theArgToken);
            if (!valid) {
              console.error(
                `Argument number ${i} (${theArgToken}) is invalid for command ${token} parser offset ${this.offset}`,
              );
              throw {
        ...
        cmdsExecutors.push(
          new CommandExecutor(cmd, args, this.afterCmdCallback),
        );
      } else {
        let endIndex = this.index + 1;
        throw {
