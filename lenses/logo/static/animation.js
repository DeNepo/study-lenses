...
(() => {
  ...
  class AnimationManager {
    ...
    update() {
      for (const anim of this.activeAnimations) anim.update();
  ...
  Animation = class {
    ...
    update() {
      ...
      for (const val in this.values)
        this.values[val] = lerp(
          this.startingValues[val],
          this.endingValues[val],
          completionRatio,
        );
      this.updateCallback(this.values);
      if (this.complete) this.finishedCallback(this.endingValues);
    }
