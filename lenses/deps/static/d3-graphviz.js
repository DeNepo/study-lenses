(function (global, factory) {
  ...
  function (
    ...
    function attributeElement(data) {
      ...
      for (
        var _i2 = 0, _Object$keys2 = Object.keys(attributes);
    ...
    function translateZoomBehaviorTransform(selection) {
      ...
      this._zoomBehavior.transform(
        this._zoomSelection,
        getTranslatedZoomTransform.call(this, selection),
      ...
      this._originalTransform = d3Zoom.zoomIdentity
        .translate(
          selection.datum().translation.x,
          selection.datum().translation.y,
        )
        .scale(selection.datum().scale);
    }
      ...
      if (this._transitionFactory) {
        d3Timer.timeout(
          function () {
            ...
            this._transition = d3Transition.transition(
              this._transitionFactory(),
            );
    ...
    function _render(callback) {
      ...
      var growEnteringEdges =
        this._options.growEnteringEdges && transitionInstance != null;
      ...
      function insertChildren(element) {
        ...
        if (
          fade ||
          (growEnteringEdges && isEdgeElementParent(element.datum()))
        ) {
          var childElementsEnter = childrenEnter
            ...
            .each(function (d) {
              ...
              for (
                var _i = 0, _Object$keys = Object.keys(d.attributes);
                _i < _Object$keys.length;
                _i++
              ) {
                var attributeName = _Object$keys[_i];
      ...
      function attributeElement(data) {
        ...
        if (data.tag == 'svg') {
          ...
          if (options.width != null || options.height != null) {
            ...
            if (!options.fit) {
              element.attr(
                'viewBox',
                '0 0 '
                  .concat((width * 3) / 4 / options.scale, ' ')
                  .concat((height * 3) / 4 / options.scale),
              );
              data.attributes.viewBox = '0 0 '
                .concat((width * 3) / 4 / options.scale, ' ')
                .concat((height * 3) / 4 / options.scale);
            }
          }
          ...
          if (
            options.scale != 1 &&
            (options.fit || (options.width == null && options.height == null))
          ) {
            ...
            element.attr(
              'viewBox',
              '0 0 '
                .concat(width / options.scale, ' ')
                .concat(height / options.scale),
            );
            data.attributes.viewBox = '0 0 '
              .concat(width / options.scale, ' ')
              .concat(height / options.scale);
        ...
        if (tweenShapes && transitionInstance) {
          if (
            (this.nodeName == 'polygon' || this.nodeName == 'ellipse') &&
            data.alternativeOld
          ) {
        ...
        if (growThisPath) {
          ...
          element
            ...
            .attr(
              'transform',
              'translate(' + data.offset.x + ',' + data.offset.y + ')',
            );
        ...
        var moveThisPolygon =
          ...
          isEdgeElement(data) &&
          data.offset &&
          data.parent.children[3].tag == 'path';

        if (moveThisPolygon) {
          var edgePath = d3.select(
            element.node().parentNode.querySelector('path'),
          );
          ...
          elementTransition
            .attrTween('transform', function () {
              return function (t) {
                ...
                var p2 = edgePath
                  .node()
                  .getPointAtLength(data.totalLength * t + 1);
                var angle =
                  (Math.atan2(p2.y - p.y, p2.x - p.x) * 180) / Math.PI - angle1;
        ...
        for (
          ...
          if (tweenThisPath && attributeName == 'd') {
            ...
            if (points) {
              elementTransition.attrTween(
                'd',
                pathTween(points, attributeValue),
              );
            }
          } else {
              ...
              if (transitionInstance) {
                ...
                elementTransition
                        ...
                        return function (t) {
                          node.setAttribute(
                            ...
                            d3Interpolate.interpolateTransformSvg(
                              d3Zoom
                                .zoomTransform(
                                  graphvizInstance._zoomSelection.node(),
                                )
                                .toString(),
                              getTranslatedZoomTransform
                                .call(graphvizInstance, element)
                                .toString(),
                            )(t),
                          );
                        };
                  ...
                  .on('end', function () {
                    ...
                    if (graphvizInstance._zoomBehavior) {
                      translateZoomBehaviorTransform.call(
                        graphvizInstance,
                        element,
                      );
                    }
                ...
                if (graphvizInstance._zoomBehavior) {
                  ...
                  attributeValue = getTranslatedZoomTransform
                    .call(graphvizInstance, element)
                    .toString();
                }
              }
      ...
      if (transitionInstance != null) {
        root
          .transition(transitionInstance)
          .on('start', function () {
            graphvizInstance._dispatch.call(
              'transitionStart',
              graphvizInstance,
            );
          })
          ...
          if (isEdgeElement(datum)) {
            if (tag == 'path' || tag == 'polygon') {
              ...
              if (prevStartNode) {
                ...
                if (i >= 0) {
                  var j = startNode.children[i].children.findIndex(function (
                    element,
                    index,
                  ) {
                ...
                var i = prevStartNode.children.findIndex(function (
                  element,
                  index,
                ) {
                  return element.tag == 'g';
        ...
        _this.on(
          ...
          enable
            ? function () {
                ...
                string +=
                  eventType + ' '.repeat(maxEventTypeLength - eventType.length);
    ...
    function insertDrawnNode(nodeId) {
      ...
      if (attributes.URL || attributes.tooltip) {
        ...
        var svgElement = a.selectWithoutDataPropagation(
          'ellipse,polygon,path,polyline',
        );
        var text = a.selectWithoutDataPropagation('text');
      } else {
        var svgElement = node.selectWithoutDataPropagation(
          'ellipse,polygon,path,polyline',
        );
        var text = node.selectWithoutDataPropagation('text');
      }
    ...
    function workerCode() {
      ...
      self.onmessage = function (event) {
        ...
        hpccWasm.graphviz
          .layout(event.data.dot, 'svg', event.data.engine, event.data.options)
          .then(function (svg) {
            if (svg) {
              self.postMessage({
                type: 'done',
    ...
    function Graphviz(selection, options) {
      ...
      if (useWorker) {
        ...
        var vizScript = scripts.filter(function () {
          return (
            d3.select(this).attr('type') == 'javascript/worker' ||
      ...
      this._dispatch = d3Dispatch.dispatch.apply(
        void 0,
        _toConsumableArray(this._eventTypes),
      );
      initViz.call(this);
    ...
    Graphviz.prototype = graphviz.prototype =
      ...
      _defineProperty(_graphviz$prototype, 'removeDrawnEdge', removeDrawnEdge),
      _defineProperty(
        _graphviz$prototype,
        'drawnEdgeSelection',
        drawnEdgeSelection,
      ...
      _defineProperty(_graphviz$prototype, 'drawNode', drawNode),
      _defineProperty(_graphviz$prototype, 'updateDrawnNode', updateDrawnNode),
      _defineProperty(_graphviz$prototype, 'moveDrawnNode', moveDrawnNode),
      _defineProperty(_graphviz$prototype, 'insertDrawnNode', insertDrawnNode),
      _defineProperty(_graphviz$prototype, 'removeDrawnNode', removeDrawnNode),
      _defineProperty(_graphviz$prototype, 'removeDrawnNode', removeDrawnNode),
      _defineProperty(
        _graphviz$prototype,
        'drawnNodeSelection',
    ...
    function selection_selectWithoutDataPropagation(name) {
      return d3.select(
        this.size() > 0 ? this.node().querySelector(name) : null,
      );
    }

    d3.selection.prototype.graphviz = selection_graphviz;
    d3.selection.prototype.selectWithoutDataPropagation =
